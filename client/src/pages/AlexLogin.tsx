...
export default function AlexLogin() {
  const [password, setPassword] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const { toast } = useToast();
  const [location, setLocation] = useLocation();

  // Senha fixa para a página Alex
  const alexPassword = "8390";
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Verifica a senha
    setTimeout(() => {
      if (password === alexPassword) {
        // Se a senha estiver correta, salva no localStorage e redireciona
        localStorage.setItem("isAlexAuthenticated", "true");
        setLocation("/alex");
        toast({
          title: "Login realizado com sucesso",
          description: "Você foi redirecionado para a área restrita",
        });
      } else {
      ...
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardDescription className="text-center">
            Digite a senha para acessar a área Alex
          </CardDescription>
        </CardHeader>
        <CardContent>
